...
string resultsDir = "test_results".testPath;
...
{
    static toolsDir = "tools".testPath;
...
auto predefinedTargets(string[] targets)
{
    ...
    {
        return dir.testPath.dirEntries("*{.d,.sh}", SpanMode.shallow).map!(e => e.name);
    }
...
string[string] getEnvironment()
{
    ...
    version(Windows)
    {
        ...
        env["DSEP"] = `\\`;
        env["SEP"] = `\`;
        auto druntimePath = environment.get("DRUNTIME_PATH", `..\..\druntime`.testPath);
        auto phobosPath = environment.get("PHOBOS_PATH", `..\..\phobos`.testPath);
        ...
        int dmdModel = `..\generated\windows\%s\64\dmd%s`.format(build, exe).testPath.exists ? 64 : 32;
        env.getDefault("MODEL", dmdModel.text);
        env["DMD"] = `..\generated\windows\%s\%d\dmd%s`.format(build, dmdModel, exe).testPath;
    }
    else
    {
        ...
        // auto-tester might run the testsuite with a different $(MODEL) than DMD
        // has been compiled with. Hence we manually check which binary exists.
        int dmdModel = "../generated/%s/%s/64/dmd".format(os, build).testPath.exists ? 64 : 32;
        env.getDefault("MODEL", dmdModel.text);

        auto generatedSuffix = "generated/%s/%s/%s".format(os, build, dmdModel);
        env["DMD"] = "../" ~ generatedSuffix ~ "/dmd".testPath;
