...
void test79()
{
    ...
    writeln(c.__vptr);
    writeln(c.__vptr[0]);
    writeln(typeid(cast(void*)c));
    assert(c.__vptr[0] == cast(void*)typeid(c));
    writeln(c.__monitor);
...
void test220()
{
  ...
  // all print 8
  writeln(typeid(T220!(int).C).init.length);
  writeln(typeid(C).init.length);
  writeln(typeid(D220).init.length);
