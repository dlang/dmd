...
{
    const bool isStaticArray = is(U == typeof(T.init));
}
...
void test45()
{
   version (none)  // this test fails in -release because asserts will be removed
   {
   assert(foo45(0)==2);
   try{
      foo45(1);
...
void test79()
{
    ...
    writefln(c.__vptr);
    writefln(c.__vptr[0]);
    writefln(cast(void*)c.classinfo);
    assert(c.__vptr[0] == cast(void*)c.classinfo);
    writefln(c.__monitor);
...
char[] foo122(char[] source) { return ""; }
const v122 = "bar";
const r122 = foo122(v122);
...
class bar134
{
    final void fun(foo134 arg = new foo134(0.)) { } 
}
...
void test139()
{
   auto famousNamedConstants = 
    [ cast(string)"pi" : 3.14, "e" : 2.71, "moving sofa" : 2.22 ];
...
{
    const isQObjectType = is(T.__isQObjectType);
}
...
{
    class C {}
    const assocarrayliteral = Q!( [1:2] ).q.stringof;
    const complex80 = Q!( 1+1.0i ).q.stringof;
    //const dottype = Q!( C.Object.toString ).q.stringof;
    const halt = (assert(0), 0).stringof;    // ICE w/ -release
    //const remove = Q!( [1:2].remove(1) ).q.stringof;
    const templat = Q!( Q ).q.stringof;
}
...
void test220()
{
  ...
  // all print 8
  writefln(T220!(int).C.classinfo.init.length);
  writefln(C.classinfo.init.length);
  writefln(D220.classinfo.init.length);
...
const real negtab[14] =
    [ 1e-4096L,1e-2048L,1e-1024L,1e-512L,1e-256L,1e-128L,1e-64L,1e-32L,
	    1e-16L,1e-8L,1e-4L,1e-2L,1e-1L,1.0L ];
const real postab[13] =
    [ 1e+4096L,1e+2048L,1e+1024L,1e+512L,1e+256L,1e+128L,1e+64L,1e+32L,
	    1e+16L,1e+8L,1e+4L,1e+2L,1e+1L ];

float parse(string p)
{
    printf("test1\n");
...
void baz5790(out int[3] y2) {
    y2[0]++;
}
void spam5790(out Foo5790 f) {
    baz5790(f.y);
}
+/
...
struct Bar7817 {
    void opAssign(Foo7817 f) {}
}

void test7817() {
    Foo7817 f;
    Bar7817 bar = f; // Broken in dmd 1.074-beta

    /* Actually, it fixes a bug, but we allow to avoid breaking existing code
     */
...
struct S9141 {
    int i;
    const s = S9141(0); // Error: more initializers than fields of S
}
