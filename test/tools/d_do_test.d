    ...
    Result testCombination(bool autoCompileImports, string argSet, size_t permuteIndex, string permutedArgs)
    {
        ...
        try
        {
            ...
            if (testArgs.postScript && !envData.coverage_build)
            {
                ...
                string cmd = "tools/postscript.sh " ~ testArgs.postScript ~ " " ~ input_dir ~ " " ~ test_name ~ " " ~ thisRunName;
                version (Windows)
                {
                    // Include the compatibility file generated by run.d to provide all missing env. variables
                    import paths : relativePosixPath;
                    const relResults = relativePosixPath(envData.results_dir, dmdTestDir);
                    cmd = "bash -c \"set -eux && source '" ~ relResults  ~ "/setup_env.sh' && source " ~ cmd ~ '"';
                }
                execute(f, cmd, true);
            }
...
int runBashTest(string input_dir, string test_name, const ref EnvData envData)
{
    ...
    version(Windows)
    {
        ...
        // Include the compatibility file generated by run.d to provide all missing env. variables
        const cmd = "bash -c \"set -eux && source '" ~ relResults  ~ "/setup_env.sh' && source " ~ script ~ ' ' ~ input_dir ~ ' ' ~  test_name ~ '"';

        auto process = spawnShell(cmd, null, Config.none, dmdTestDir);
    }
    else
    {
