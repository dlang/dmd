...
unittest
{
    const udas = extractUDALineNumber(q{
        void foo()
        {
            @("foo")
            @("bar")
    ...
    import std.stdio;writeln(udas);
    assert(udas == [UDA(4, "foo"), UDA(5, "bar")]);
}
...
UDA[] extractUDALineNumber(string code,
    DiagnosticReporter diagnosticReporter = null)
{
    ...
    extern (C++) static class Visitor : SemanticTimeTransitiveVisitor
    {
        ...
        {
            foreach (attrib; *uda.getAttributes)
            {
                auto tuple = attrib.toTupleExp;
                const value = (*tuple.exps)[0].toStringExp.peekSlice;

                udas ~= UDA(uda.loc.linnum, value);
            }

            // writefln!"uda_length=%s tuple_length=%s"(uda.atts.length, tuple.exps.length);
        }
    }
