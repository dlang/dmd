...
extern(C) nothrow
{
    ...
    void _d_valgrind_get_vbits(const(void)* addr, ubyte* bits, size_t len);
    void _d_valgrind_set_vbits(const(void)* addr, ubyte* bits, size_t len);
...
void getVBits(const(void)[] mem, ubyte[] bits) nothrow
{
    assert(mem.length == bits.length);
    _d_valgrind_get_vbits(mem.ptr, bits.ptr, mem.length);
}

void setVBits(const(void)[] mem, ubyte[] bits) nothrow
{
    assert(mem.length == bits.length);
