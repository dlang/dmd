...
test_dmd() {
    ...
    if [ "$FULL_BUILD" == "true" ] && [ "$OS_NAME" == "linux"  ]; then
        make -j1 -C test auto-tester-test unit_tests MODEL=$MODEL N=$N # all ARGS by default
    else
        make -j1 -C test auto-tester-test unit_tests MODEL=$MODEL N=$N ARGS="-O -inline -release"
    fi
}
