...
grep "_d_arraysetlengthT!(int\\[\\], int)(arr, 100.*)" "${TEST_DIR}/${TEST_NAME}.d.cg" &&
# Make sure there's no call to the runtime for .length = 0
! grep "_d_arraysetlengthT!(int\\[\\], int)(\(arr\|f\), 0.*)" "${TEST_DIR}/${TEST_NAME}.d.cg" &&
# Make sure there's no call to the runtime for .length = x
! grep "_d_arraysetlengthT!(int\\[\\], int)(f, x.*)" "${TEST_DIR}/${TEST_NAME}.d.cg" &&
