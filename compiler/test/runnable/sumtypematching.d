...
__sumtype WithEmpty = Empty | bool;
static assert(WithEmpty.sizeof == 2); // tag(1) + union(1) = 2

__sumtype WithEmptyLarge = Empty | int;
...
/// Form 1: Block declaration form (unnamed variants)
...
void testMatchMultiVariant()
{
    ...
    auto r = s.match
    {
        (int x) => x + 1, (bool y) => 99, (string z) => cast(int) z.length
    };
...
void testCatchAll()
{
    ...
    auto r = s.match
    {
        (int x) => x * 2, (y) =>  - 1 // catch-all: y is bool or string depending on variant
    };
    ...
    auto r2 = s2.match
    {
        (int x) => x * 2, (y) =>  - 1 // catch-all: matches string variant
    };
...
int takeWideAndMatch(S2 s)
{
    return s.match
    {
        (int x) => x * 2, (bool y) => y ? 1 : 0, (string z) => cast(int) z.length
    };
}
...
void testGuardCatchAllWithGuardedTyped()
{
    ...
    {
        (int v)if (v > 100)
             => v, (z) =>  - 1 // catch-all: reached when int guard fails
    };
    assert(r ==  - 1);
...
__sumtype LS1 = Copier | int;
__sumtype LS2 = PostBlit | int;
__sumtype LS3 = Tracked | int;
...
void testMatchRefNoCopy()
{
...

    {
        ...
        s.match
        {
            (ref Tracked t) => (t.val = 99, 0), (int i) => 0
        };
...
void testCrossSumtypeLifecycle()
{
    ...
    __sumtype CST1 = PostBlit | int;
    __sumtype CST2 = PostBlit | int | string;
...
void testOpCmpToHash()
{
    ...
    {
        int x;
        @disable int opCmp(ref const DisCmp) const;
    }

    __sumtype NoCmp = int | DisCmp;
    __sumtype BoolSum = int | bool;
...
//   S!(int, string)  ==  __sumtype(int | string | bool) [inline form removed]
void testTemplatesAndAliasSequences()
{
    ...
    auto r = b.match
    {
        (int x) => x + 1, (string s) => cast(int) s.length, (bool z) => 0
    };
    assert(r == 5);
    ...
    __sumtype DirectTpl = int | string | bool;
...
// ============================================================
// Section 19: Match Result Type (Sumtype)
// ============================================================

__sumtype Src19 = int | string;

void testMatchResultSumtype()
{
    Src19 s = Src19(42);

    // int + uint arms should unify to a single type (long) via integer promotion
    auto r = s.match
    {
        (int x) => cast(long) x * 2, (string y) => cast(uint) y.length
    };
    static assert(is(typeof(r) == long));
    assert(r == 84);
    printf("  match result int+uint -> long: %lld\n", r);
}

void testMatchResultSingleType()
{
    Src19 s = Src19(10);

    // Both arms return int, so result is just int (no sumtype)
    auto r = s.match
    {
        (int x) => x * 2, (string y) => cast(int) y.length + 1
    };
    static assert(is(typeof(r) == int));
    assert(r == 20);
    printf("  match result single type: %d\n", r);
}

__sumtype Src19b = int | bool | string;

void testMatchResultMultiVariant()
{
    Src19b s = Src19b("hello");

    // Arms return int and string, so result is a sumtype
    auto r = s.match
    {
        (int x) => x * 2, (bool y) => 42, (string z) => z ~ "!\0"
    };
    // Result should be a sumtype (int | string)
    static assert(is(typeof(r) == __sumtype));
    assert(r.tag == 1); // string variant
    assert(r.__v1 == "hello!\0");
    printf("  match result multi-variant: tag=%d val=%s\n", r.tag, r.__v1.ptr);
}
...
void main()
{
    ...
    printf("[19] Match Result Type (Sumtype)\n");
    testMatchResultSumtype();
    testMatchResultSingleType();
    testMatchResultMultiVariant();
