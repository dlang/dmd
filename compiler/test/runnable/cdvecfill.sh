...
if [ $OS == "linux" ] && [ $MODEL == "64" ]; then
  $DMD -betterC -c -O -m64 ${src_file} -of${obj_file} -check=nullderef=off
  ...
  $DMD -betterC -c -O -m64 -mcpu=avx ${src_file} -of${obj_file} -check=nullderef=off
  objdump --disassemble --disassembler-options=intel ${obj_file} | tail -n+3 | sed 's/[ \t]\s*$//' > ${tmp_file}
  diff ${expect_file_avx} ${tmp_file}
  rm_retry ${obj_file} ${tmp_file}

  $DMD -betterC -c -O -m64 -mcpu=avx2 ${src_file} -of${obj_file} -check=nullderef=off
  objdump --disassemble --disassembler-options=intel ${obj_file} | tail -n+3 | sed 's/[ \t]\s*$//' > ${tmp_file}
