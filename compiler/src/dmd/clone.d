...
FuncDeclaration hasIdentityOpAssign(AggregateDeclaration ad, Scope* sc)
{
    ...
    if (assign)
    {
        ...
        a[0] = er;
        auto f = resolveFuncCall(ad.loc, sc, assign, TemplateArguments(), ad.type, ArgumentList(&a), FuncResolveFlag.quiet);
        if (!f)
        {
            a[0] = el;
            f = resolveFuncCall(ad.loc, sc, assign, TemplateArguments(), ad.type, ArgumentList(&a), FuncResolveFlag.quiet);
        }
    ...
    if (Dsymbol eq = search_function(ad, Id.eq))
    {
        ...
        bool hasIt(Type tthis)
        {
            ...
            FuncDeclaration rfc(Expression e)
            {
                a[0] = e;
                a[0].type = tthis;
                return resolveFuncCall(ad.loc, sc, eq, TemplateArguments(), tthis, ArgumentList(&a), FuncResolveFlag.quiet);
            }
