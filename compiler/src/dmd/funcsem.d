...
/*****************************************
 * Determine lexical level difference from `fd` to nested function `fd2`.
 * Params:
 *      fd = location of call
 *      fd2 = target of call
 ...
 */
extern (D) int getLevel(FuncDeclaration fd, FuncDeclaration fd2, int intypeof)
{
    //printf("FuncDeclaration::getLevel(fd = '%s')\n", fd.toChars());
    Dsymbol fdparent = fd2.toParent2();
    if (fdparent == fd)
        return -1;

    Dsymbol s = fd;
    int level = 0;
    while (fd2 != s && fdparent != s.toParent2())
    {
        //printf("\ts = %s, '%s'\n", s.kind(), s.toChars());
