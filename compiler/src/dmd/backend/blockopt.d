...
import dmd.backend.symbol : symbol_genauto;
import dmd.backend.var : OPTIMIZER, debugb, debugc, debugw, globsym, sytab;
...
void blockopt(ref GlobalOptimizer go, ref BlockOpt bo)
{
    if (OPTIMIZER)
    {
//xyzzy        blassertsplit(go, bo);              // only need this once
        ...
        do
        {
            ...
            go.changes = 0;
//xyzzy            bropt(go, bo);                  // branch optimization
            ...
            if (bc == BC.iftrue)
            {
                        ...
                        )
                {
                    ...
                    anychanges++;
                         ...
                         b3.bc == BC.goto_ &&
                         b2.Bsucc[0] is b3.Bsucc[0])
                {
                    block* bsucc = b2.Bsucc[0];
                    if (b2.Belem)
                    {
                        elem* e;
                        if (b3.Belem)
...
void compdfo(ref BlockOpt bo, ref Barray!(block*) dfo, block* startblock)
{
    ...
    void walkDFO(block* b)
    {
        ...
        foreach (bs; b.Bsucc[])              // for each successor
        {
            assert(bs);
            if ((bs.Bflags & BFL.visited) == 0) // if not visited
                walkDFO(bs);
    ...
    for (block** pb = &bo.startblock; (b = *pb) != null;)
    {
            ...
            )
        {
            ...
            foreach (s; b.Bsucc[])
            {
                ...
                {
                    bool result = s.Bpred.subtract(b);
                    assert(result);
                }
            }
    ...
    for (block* bn = bo.startblock; bn; bn = bnext)
    {
        ...
        for (block* b = bnext; b; b = b.Bnext)
        {
            ...
            static if (SCPP_OR_NTEXCEPTIONS)
                ...
                enum additionalAnd = true;
            if (b.bc == bn.bc &&
                //(!OPTIMIZER || !(go.mfoptim & MFtime) || !b.Bsucc.length) &&
...
private void brtailrecursion(ref GlobalOptimizer go, ref BlockOpt bo)
{
    ...
    for (block* b = bo.startblock; b; b = b.Bnext)
    {
        ...
        if (*pe &&
            (b.bc == BC.ret ||
             b.bc == BC.retexp ||
             (b.bc == BC.goto_ && (bn = b.Bsucc[0]).Belem == null &&
              bn.bc == BC.ret)
            )
           )
        {
