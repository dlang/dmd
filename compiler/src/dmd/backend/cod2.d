...
void cdcond(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    {   // Recognize (e ? c : f)
        ...
        const reg = regwithvalue(cdb,retregs,cast(targ_size_t)e21.EV.Vllong,
...
void cdind(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    L1:
        ...
        {
            cs.Iop = 0x8B;                                  // MOV
            if (sz <= 2 && !I16 && config.flags4 & CFG4speed)
            {
...
if (config.exe & EX_windos)
{
    ...
    else if (sz <= REGSIZE || tyfv(tyml))
    {
        ...
        //      MOV     EA,reg
        //      ~OP     reg
        if (sz <= REGSIZE && (cs.Irm & 0xC0) != 0xC0 && config.flags4 & CFG4speed)
        {
