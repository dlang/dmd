...
import dmd.backend.blockopt : block_calloc, blockopt, compdfo;
import dmd.backend.cgelem : doptelem, elemisone;
...
L1:
    ...
    foreach (b; head.Bpred[])
    {
        if (!vec_testbit(b.Bdfoidx,l.Lloop))  /* if not in loop       */
        {
            ...
            {
                if (b.Bsucc.length > 1)       // if more than 1 successor
  ...
  L3:
    ...
    foreach (ref l; startloop)
    {
        ...
        if (!l.Lpreheader)             /* if no preheader               */
        {
            ...
            l.Lpreheader = p;
            p.bc = BC.goto_;
            assert(p.Bsucc.length == 0);
            p.Bsucc.push(h); /* only successor is h          */
            p.Btry = h.Btry;
