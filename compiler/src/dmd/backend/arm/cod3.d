          ...
          L1:
            switch (c.IFL2)
            {
                case FLblock:
                     ...
                     * closer.
                     */
                    if (disp >= (1 << 22) - 5 && c.IEV2.Vblock.Boffset > offset)
...
printf("c: %p, x%08x\n", c, c.Iop);
                printf("s = %s, Soffset = %d, Para.size = %d, BPoff = %d, EBPtoESP = %d, imm12 = %d, Vpointer = %d, Voffset = %d\n",
                    s.Sident.ptr, cast(int)s.Soffset, cast(int)cgstate.Para.size, cast(int)cgstate.BPoff,
                    cast(int)cgstate.EBPtoESP, (c.Iop >> 10) & 0xFFF, cast(int)c.IEV1.Vpointer, cast(int)c.IEV1.Voffset);
                c.Iop += (s.Soffset + soff + cgstate.BPoff) << 10;      // add to imm12
                if (s.Sflags & SFLunambig)
                    c.Iflags |= CFunambig;
            L2:
                if (!cgstate.hasframe || (cgstate.enforcealign && c.IFL1 != FLpara))
                {   /* Convert to SP relative address instead of BP */
                    c.Iop += (cgstate.EBPtoESP >> (c.Iop & 0x8000_0000 ? 3 : 2)) << 10; // add to imm12
                    c.Iop |= 0x1F << 5; // set Rn to SP
                }
                break;
