 ...
 * Source:      $(LINK2 https://github.com/dlang/dmd/blob/master/src/dmd/backend/arm/cod3.d, backend/cod3.d)
 * Documentation:  https://dlang.org/phobos/dmd_backend_arm_cod3.html
 * Coverage:    https://codecov.io/gh/dlang/dmd/src/master/src/dmd/backend/arm/cod3.d
 */
...
{
    // Only handles integer regs at the moment
...
void movregconst(ref CodeBuilder cdb,reg_t reg,targ_size_t value,regm_t flags)
{
    ...
    else
    {
        ...
        if ((value >> (hw * 16)) & 0xFFFF_FFFF_FFFF_0000)
        {
            // Check for ORR one instruction solution
            uint N, immr, imms;
            if (orr_solution(value, N, immr, imms))
            {
                // MOV Rd,#imm
...
void assignaddrc(code *c)
{
    ...
    targ_size_t base = cgstate.EBPtoESP;
    for (; c; c = code_next(c))
    {
        debug
        {
        ...
        switch (c.IFL1)
        {
            ...
            L1:
                ...
                static if (0)
                {
                    symbol_print(*s);
                    printf("c: %p, x%08x\n", c, c.Iop);
                    printf("s = %s, Soffset = %d, Para.size = %d, BPoff = %d, EBPtoESP = %d, Vpointer = %d, Voffset = %d\n",
                        s.Sident.ptr, cast(int)s.Soffset, cast(int)cgstate.Para.size, cast(int)cgstate.BPoff,
                        cast(int)cgstate.EBPtoESP, cast(int)c.IEV1.Vpointer, cast(int)c.IEV1.Voffset);
                }
                imm12 = cast(uint)(s.Soffset + soff + cgstate.BPoff);      // add to imm12
                imm12 += localsize + 16;
                Rn = (c.Iop >> 5) & 0x1F;
                Rt = c.Iop & 0x1F;
            ...
            L2:
                ...
                {   /* Convert to SP relative address instead of BP */
                    assert(Rn == 29); // BP
                    imm12 += cgstate.EBPtoESP; // add difference in offset
                    Rn = 31; // set Rn to SP
                }
                static if (0)
                {
                ...
                c.Iop = INSTR.ldr_imm_gen(is64, Rt, Rn, imm12); // LDR Rt,Rn,#imm12
                break;
