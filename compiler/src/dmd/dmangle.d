...
extern (C++) void mangleToBuffer(Expression e, OutBuffer* buf)
{
    ...
    auto backref = Backref(null);
    mangleExpression(e, buf, backref);
}
        ...
        for (size_t i = 0; i < args.dim; i++)
        {
            ...
            else if (ea)
            {
                ...
                */
                ...
                mangleExpression(ea, buf, *backref);
            }
            else if (sa)
            {
            Lsa:
...
/*************************************************************
 */
void mangleExpression(Expression e, OutBuffer* buf, ref Backref backref)
{
    void mangleSymbol(Dsymbol s)
    {
        scope Mangler v = new Mangler(buf, &backref);
    ...
    void visitArrayLiteral(ArrayLiteralExp e)
    {
        ...
        foreach (i; 0 .. dim)
        {
            e[i].mangleExpression(buf, backref);
        }
    }
    ...
    void visitAssocArrayLiteral(AssocArrayLiteralExp e)
    {
        ...
        foreach (i; 0 .. dim)
        {
            (*e.keys)[i].mangleExpression(buf, backref);
            (*e.values)[i].mangleExpression(buf, backref);
        }
    }
    ...
    void visitStructLiteral(StructLiteralExp e)
    {
        ...
        foreach (i; 0 .. dim)
        {
            ...
            if (ex)
                ex.mangleExpression(buf, backref);
            else
    ...
    switch (e.op)
    {
        case EXP.int64:             visitInteger          (e.isIntegerExp);           break;
        case EXP.float64:           visitReal             (e.isRealExp);              break;
        case EXP.complex80:         visitComplex          (e.isComplexExp);           break;
        case EXP.null_:             visitNull             (e.isNullExp);              break;
        case EXP.string_:           visitString           (e.isStringExp);            break;
        case EXP.arrayLiteral:      visitArrayLiteral     (e.isArrayLiteralExp);      break;
        case EXP.assocArrayLiteral: visitAssocArrayLiteral(e.isAssocArrayLiteralExp); break;
        case EXP.structLiteral:     visitStructLiteral    (e.isStructLiteralExp);     break;
        case EXP.function_:         visitFunc             (e.isFuncExp);              break;
