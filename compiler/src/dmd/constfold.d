...
UnionExp Add(const ref Loc loc, Type type, Expression e1, Expression e2)
{
    ...
    static real_t addf(real_t x, real_t y) { pragma(inline, false); float  f = cast(float)x + cast(float)y; return cast(real_t) f; }
    static real_t addd(real_t x, real_t y) { pragma(inline, false); double d = cast(double)x + cast(double)y; return cast(real_t) d; }
