...
final class CParser(AST) : Parser!AST
{
    ...
    AST.Statement cparseStatement(int flags, const(char)** endPtr = null, Loc* pEndloc = null)
    {
        ...
        case TOK.leftBracket:
            /* C23 6.7.13 attribute-specifier-sequence prefixing a declaration or
             * statement, e.g. `[[maybe_unused]] int x;` or `[[fallthrough]];`
             */
            if (peekNext() != TOK.leftBracket)
                goto default;       // a lone `[` is not an attribute
            {
                Specifier attrspec;
    ...
    Lwhile:
        while (1)
        {
            ...
            switch (token.value)
            {
                ...
                case TOK.leftBracket:
                {
                    /* C23 6.7.13 attribute-specifier-sequence:
                     *    declaration-specifiers (opt) attribute-specifier-sequence
                     */
                    if (peekNext() != TOK.leftBracket)
                        goto default;       // a lone `[` is not an attribute
