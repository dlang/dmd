...
extern (D) bool checkClosure(FuncDeclaration fd)
{
    ...
    foreach (v; fd.closureVars)
    {
        ...
        if (auto ts = v.type.isTypeStruct())
        {
            ...
            {
                eSink.error(v.loc, "variable `%s` has scoped destruction, cannot build closure", v.toPrettyChars());
                fd.errors = true;
                return true;
            }
        }
        else if (auto tc = v.type.isTypeClass())
        {
            ...
            {
                eSink.error(v.loc, "scoped class variable `%s` has destructor, cannot build closure", v.toPrettyChars());
                fd.errors = true;
                return true;
            }
        }
