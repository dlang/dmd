...
void reportLoopyLabelLessNullThan(DFACommon* dfaCommon, DFAVar* var, ref Loc loc)
{
    if ((var !is null && !var.isModellable) || (var !is null && !var.isModellable))
        return;
...
void reportLoopyLabelAssertAndAssign(DFACommon* dfaCommon,
        DFAScopeVar* varThatWasAssumed, DFAScopeVar* varThatAsserted, ref Loc loc)
{
...
void reportEndOfFunction(DFACommon* dfaCommon, FuncDeclaration fd, ref Loc loc)
{
    ...
    foreachFunctionVariable(dfaCommon, fd, (scv) {
        ...
        version (none)
        {
            ...
            printf("%d %d:%d:%d %d\n", scv.var.unmodellable, scv.var.isAPointer,
                scv.var.isByRef, scv.var.writeCount, param.specifiedByUser);
