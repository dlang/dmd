...
struct DFACommon
{
    ...
    DFAVar* findDereferenceVar(DFAVar* childOf)
    {
        ...
        if (childOf.dereferenceVar is null)
        {
            ...
            childOf.dereferenceVar.youngestLifeTimeAllowedDepth
                = childOf.youngestLifeTimeAllowedDepth;
            childOf.dereferenceVar.oldestLifeTimeAllowedDepth = childOf.oldestLifeTimeAllowedDepth;
        }
...
struct DFAVar
{
    ...
    void visitIndirectSources(scope void delegate(DFAVar* var,
            ...
            bool takenAddressOf, bool hadFields, bool isOffsetOfStorage, ref bool unknown) del)
    {
        ...
        // We have to be able to model the following code:
        // *(*var).field
        // The explicit deref inside of the brackets, that is an inner deref, whereas the one on outside is an outer deref.
        // It is important to be able to know this, for reads.
        // The D compiler treats a lot of AST nodes as references, they can result in pointers to the thing,
        //  or it can be a read of the thing direction.
        // Dereferencing is handled separately from reading,
        //  so some algorithms need to differentiate what has already been handled and what hasn't been.

        bool unknown;

        void findStorage(DFAVar* var, bool haveInnerDeref, bool haveOuterDeref,
                int indirectCount, bool takenAddressOf, int fieldResolve, int isOffsetOfStorage)
        {
            if (unknown)
                return;

            bool trackInnerDefAs = haveInnerDeref;
            ...
            while (var !is null)
            {
                ...
                if (var.base1 !is null && var.base1.dereferenceVar is var)
                {
                    var = var.base1;
                    haveInnerDeref = true;
                }
                ...
                {
                    haveInnerDeref = trackInnerDefAs;
                    trackInnerDefAs = false;
                }
                ...
                if (var.base1 !is null)
                {
                    if (var.base1.indexVar is var && indirectCount == 0)
                        isOffsetOfStorage++;
                    else if (var.base1.addressOfVar is var || var.base1.asSliceVar is var)
                    {
                ...
                if (var.base2 !is null)
                {
                    findStorage(var.base2, haveInnerDeref, haveOuterDeref,
                            indirectCount, takenAddressOf, fieldResolve, isOffsetOfStorage);
                    findStorage(var.base1, haveInnerDeref, haveOuterDeref,
                            indirectCount, takenAddressOf, fieldResolve, isOffsetOfStorage);
                    return;
                }
            ...
            del(var, indirectCount > 0, haveInnerDeref, haveOuterDeref,
                    takenAddressOf, fieldResolve > 0, isOffsetOfStorage > 0, unknown);
        }

        void findOffset(DFAVar* var, int fieldResolve, int isOffsetOfStorage)
        {
            bool foundTakeAddressOf, haveOuterDeref;
            ...
            while (var !is null)
            {
                ...
                if (var.base2 !is null)
                {
                    findOffset(var.base2, fieldResolve, isOffsetOfStorage);
                    findOffset(var.base1, fieldResolve, isOffsetOfStorage);
                    return;
                }
            ...
            findStorage(var, haveOuterDeref, haveOuterDeref, 0,
                    foundTakeAddressOf, fieldResolve, isOffsetOfStorage);
        }
...
struct DFAPAValue
{
    ...
    /**
        Meet of two point analysis values that may be ugpraded from an unknown state.
    */
    DFAPAValue upgradableMeet(DFAPAValue other)
    {
        if (this.kind != other.kind || this.kind != Kind.Concrete)
            return Unknown;
        ...
        return DFAPAValue(ret);
    }

    /**
        Meet of two point analysis values that won't upgrade an Unknown.
    */
...
struct DFAConsequence
{
    ...
    /**
        Performs a meet operation that will upgrade from an unknown state.
    */
    void upgradableMeetConsequence(DFAConsequence* other)
    {
        if (other is null)
            return;
        ...
        version (DebugJoinMeetOp)
        {
            printf("meet upgrade %p %d %d %d\n", other.var, other.truthiness,
                    other.nullable, other.writeOnVarAtThisPoint);
            fflush(stdout);
        }

        if (this.truthiness == Truthiness.Unknown)
            this.truthiness = other.truthiness;
        else if (this.truthiness > other.truthiness)
            this.truthiness = other.truthiness;

        if (this.nullable == Nullable.Unknown)
            this.nullable = other.nullable;
        else if (this.nullable > other.nullable)
            this.nullable = other.nullable;

        if (other.invertedOnce)
            this.invertedOnce = true;

        if (other.protectElseNegate)
            this.protectElseNegate = true;

        this.pa = this.pa.upgradableMeet(other.pa);

        if (this.obj !is null && other.obj !is null)
            this.obj = dfaCommon.makeObject(this.obj, other.obj);
        else if (other.obj !is null)
            this.obj = other.obj;
    }

    /**
        Performs a meet operation on two, three consequences.

        Do not use this when you need to upgrade from an unknown state.
    */
