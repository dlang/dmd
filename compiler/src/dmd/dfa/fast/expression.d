    ...
    DFALatticeRef walk(Expression expr)
    {
        ...
        case EXP.call:
            ...
            if (ce.e1 !is null)
            {
                ...
                this.seeConvergeFunctionCallArgument(thisExp, info, 0, ce.f, ce.e1.loc);
            }
            ...
            if (ce.arguments !is null && ce.arguments.length > 0)
            {
                ...
                VarDeclaration lastParameterVar;
                ...
                if (ce.f.parameters !is null && ce.f.parameters.length != 0)
                {
                    allParameterVars = (*ce.f.parameters)[];
                    lastParameterVar = allParameterVars[$ - 1];
                }

                this.print("Calling function parameter info\n");
                this.print("    count=%zd, calledFunctionInfo=%p:%zd\n", ce.arguments.length,
                        calledFunctionInfo !is null ? calledFunctionInfo.parameters.length : 0);
                this.print("      allParameters=%zd, lastParameter=%p, stc=%lld\n", allParameters.length,
                        lastParameter, lastParameter !is null ? lastParameter.storageClass : 0);
                this.print("      allParameterVars=%zd, lastParameterVar=%p, stc=%lld\n",
                        allParameterVars.length, lastParameterVar,
                        lastParameterVar !is null ? lastParameterVar.storage_class : 0);
                ...
                if (calledFunctionInfo is null || calledFunctionInfo.parameters.length == 0)
                {
                    ...
                    foreach (i, arg; *ce.arguments)
                    {
                        ...
                        VarDeclaration paramVar = i >= allParameterVars.length
                            ? lastParameterVar : allParameterVars.ptr[i];
                        const storageClass = (param !is null ? param.storageClass : 0) | (paramVar !is null
                                ? paramVar.storage_class : 0);

                        this.print("Argument %zd, param %p, paramVar %p\n", i, param, paramVar);

                        DFALatticeRef argExp = this.walk(arg);
                        this.seeConvergeFunctionCallArgument(argExp, null,
                                storageClass, ce.f, ce.e1.loc);
                ...
                else
                {
                    ...
                    foreach (i, arg; *ce.arguments)
                    {
                        ...
                        this.print("Argument %zd, stc=%lld\n", i, storageClass);
                        if (info !is null)
        ...
        case EXP.index:
            ...
            if (ie.modifiable && lhsType.isTypeAArray !is null)
            {
                ...
                // In this case the effect is on the lhs context, not index var
                lhsCtxHasEffect = true;
            }
            ...
            DFAConsequence* newCctx = combined.addConsequence(indexVar), effectCctx = newCctx;
            combined.setContext(newCctx);

            if (lhsCtxHasEffect)
                effectCctx = combined.addConsequence(lhsCtx);

            effectCctx.nullable = expectedNullable;
            effectCctx.truthiness = expectedTruthiness;
        ...
        case EXP.concatenateDcharAssign:
            ...
            print("concat nullable result %d\n", nullableResult);
