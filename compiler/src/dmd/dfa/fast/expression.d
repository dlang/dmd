...
struct ExpressionWalker
{
    ...
    DFALatticeRef walk(Expression expr)
    {
        ...
        case EXP.cast_:
            return this.walk(expr.isCastExp.e1);

        case EXP.equal:
            auto be = expr.isBinExp;
            return seeEqual(walk(be.e1), walk(be.e2), true, false);

        case EXP.notEqual:
            auto be = expr.isBinExp;
            return seeEqual(walk(be.e1), walk(be.e2), false, false);

        case EXP.identity: // is
        case EXP.is_: // is
            auto be = expr.isBinExp;
            return seeEqual(walk(be.e1), walk(be.e2), true, true);

        case EXP.notIdentity: // !is
            auto be = expr.isBinExp;
            return seeEqual(walk(be.e1), walk(be.e2), false, true);
        ...
        case EXP.call:
            ...
            if (calledFunctionInfo.returnValue.notNullOut == Fact.Guaranteed)
            {
        ...
        case EXP.assign:
            ...
            DFALatticeRef lhs = this.walk(ae.e1), rhs = this.walk(ae.e2);
        ...
        case EXP.index:
            ...
            DFALatticeRef lhs = this.walk(ie.e1), index = this.walk(ie.e2);

            if (ie.indexIsInBounds)
            {
                DFAConsequence* cctx = lhs.getContext;

                if (cctx !is null)
                {
                    if (cctx.nullable != Nullable.NonNull)
                    {
            ...
            DFALatticeRef ret = seeDereference(ie.loc, lhs);
            return ret;
        ...
        case EXP.min:
            ...
            if (cctxLHS !is null && cctxLHS.var !is null && cctxLHS.var.isAPointer)
                changeCtxTo = cctxLHS.var;

            DFALatticeRef ret = seeLogicalAnd(lhs, rhs);
            if (ret.isNull)
                ret = dfaCommon.makeLatticeRef;
            if (changeCtxTo !is null)
                ret.setContext(changeCtxTo);

            DFAConsequence* c = ret.getContext;
            c.truthiness = Truthiness.Unknown;
            return ret;
