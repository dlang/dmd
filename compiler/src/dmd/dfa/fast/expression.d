...
struct ExpressionWalker
{
     ...
     * create a scope where `x` is known to be null.
     */
    DFALatticeRef seeEqualIdentity(DFALatticeRef lhs, DFALatticeRef rhs, bool truthiness,
        Type lhsType, Type rhsType)
    {
        const equalityType = equalityArgTypes(lhsType, rhsType);

        dfaCommon.printState((ref OutBuffer ob, scope PrintPrefixType prefix) {
            ob.printf("Equal identity iff=%d, ty=%d:%d, ety=%d", truthiness, lhsType.ty,
            rhsType !is null ? rhsType.ty : -1, equalityType);
            ob.writestring("\n");
        });
    ...
    DFALatticeRef entry(DFAVar* assignTo, DFAVar* constructInto, Expression exp)
    {
        ...
        if (assignTo !is null)
            return seeAssign(assignTo, false, lattice, exp.loc);
        else if (constructInto !is null)
            return seeAssign(constructInto, true, lattice, exp.loc);
        else
            return lattice;
    }
