...
struct ExpressionWalker
{
    ...
    DFALatticeRef walk(Expression expr)
    {
        ...
        case EXP.lessOrEqual:
            // We do not support the full op, but we can do a little bit.
            // lhs < rhs = Maybe
            // lhs <= rhs = Maybe
            ...
            if (auto ale = be.e2.isArrayLengthExp)
            {
                // 0 < slice.length

                // this should be the length of (rhsBaseVar)
                DFAVar* rhsVar = rhs.getContextVar;
                DFAVar* rhsBaseVar = rhsVar !is null ? rhsVar.base1 : null;

                ret = seeLogicalAnd(lhs, rhs);
                if (ret.isNull)
                    ret = dfaCommon.makeLatticeRef;

                ret.acquireConstantAsContext(Truthiness.Maybe, Nullable.Unknown);

                if (rhsBaseVar !is null)
                {
                    DFAConsequence* c = ret.addConsequence(rhsBaseVar);
                    c.truthiness = Truthiness.True;
                    c.nullable = Nullable.NonNull;
                }
                else
        ...
        case EXP.greaterOrEqual:
            ...
            {
                // slice.length > 0
                // this should be the length of (lhsBaseVar)
                DFAVar* lhsVar = lhs.getContextVar;
                DFAVar* lhsBaseVar = lhsVar !is null ? lhsVar.base1 : null;

                ret = seeLogicalAnd(lhs, rhs);
                if (ret.isNull)
                    ret = dfaCommon.makeLatticeRef;

                if (lhsBaseVar !is null)
                {
                    DFAConsequence* c = ret.addConsequence(lhsBaseVar);
                    c.truthiness = Truthiness.True;
                    c.nullable = Nullable.NonNull;
                }
                else
        ...
        case EXP.question:
            ...
            DFALatticeRef conditionLR = this.walkCondition(qe.econd, predicateNegation);
