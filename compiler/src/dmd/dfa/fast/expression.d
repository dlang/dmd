...
struct ExpressionWalker
{
    ...
    DFALatticeRef seeConditional(DFALatticeRef condition, DFAScopeRef scrTrue,
            ...
            bool unknownBranchTaken)
    {
        ...
        DFALatticeRef ret = analyzer.transferConditional(condition, scrTrue,
                lrTrue, scrFalse, lrFalse, unknownBranchTaken);
        dfaCommon.check;
    ...
    DFALatticeRef conditional(Expression expr, bool truthiness)
    {
        if (expr is null)
            return DFALatticeRef.init;
        return this.conditional(this.walk(expr), expr.type, truthiness);
    }

    DFALatticeRef conditional(DFALatticeRef ret, Type type, bool truthiness)
    {
        if (ret.isNull)
        {
    ...
    DFALatticeRef walk(Expression expr)
    {
        ...
        case EXP.cast_:
            ...
            if (ce.to is null || ce.to.isTypeClass)
            {
                ...
                cctx = ret.acquireConstantAsContext(Truthiness.Unknown, Nullable.Unknown);
            }
        ...
        case EXP.assign:
            ...
            dfaCommon.printStateln("assign converge");
        ...
        case EXP.andAnd:
            ...
            stmtWalker.seeConvergeScope(scr);
        ...
        case EXP.orOr:
            ...
            while (be !is null)
            {
                ...
                toNegate = this.conditional(lhs.copy, be.e1.type, false);
            ...
            stmtWalker.seeConvergeScope(inProgressScr);
            return inProgress;
        ...
        case EXP.prePlusPlus:
            ...
            DFALatticeRef ret = seeAssign(lhs, false, rhs, false, (lhsTruthy == Truthiness.False
                    && rhsTruthy == Truthiness.True) || (lhsTruthy == Truthiness.True
                    && rhsTruthy > Truthiness.Maybe) ? 1 : 0);
            if (ret.isNull)
                ret = dfaCommon.makeLatticeRef;

            DFALatticeRef effect = dfaCommon.makeLatticeRef;
            DFAConsequence* cctx = effect.addConsequence(lhsVar, ret.findConsequence(lhsVar));
        ...
        case EXP.question:
            ...
            {
                condition = this.conditional(qe.econd, true);
                ...
                if (!ignoreTrueBranch)
                    this.seeAssert(condition.copy, qe.econd.loc, true);
            }
...

            {
                ...
                {
                    DFALatticeRef condition2 = this.conditional(qe.econd, false);
                    this.seeSilentAssert(condition2, false, true);
                ...
                {
                    DFALatticeRef condition2 = this.conditional(qe.econd, false);
                    this.seeSilentAssert(condition2, true, true);
                }
            ...
            return this.seeConditional(condition, scrTrue, lrTrue, scrFalse,
                    lrFalse, unknownBranchTaken);
