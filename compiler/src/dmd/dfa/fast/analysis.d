...
struct DFAAnalyzer
{
    ...
    void convergeExpression(DFALatticeRef lr)
    {
        ...
        if (var.base is null)
            dfaCommon.swapLattice(var, lattice => join(lattice, lr));
    }
            ...
            Parameter paramVar, FuncDeclaration calling, ref Loc loc)
    {
        ...
        // This was likely called cyclically, we do not have any DFA info for it.
        // So we'll set everything to unknown, except the other consequences.

        const isNotByRef = paramVar !is null && (paramVar.storageClass & STC.ref_) != STC.ref_;

        DFAVar* seePointer(DFAVar* var)
        {
            if (isNotByRef && var.offsetFromBase == -1)
                return var;
        ...
        // If the parameter is by-ref that means it could've been modified.
        // We'll set all state to unknown for the maybe tops.
    ...
    DFAScopeRef transferLoopyLabelStage(DFAScopeRef previous, DFAScopeRef next)
    {
        if (previous.isNull)
            return next;
...
private:
    ...
    DFAScopeRef meetScope(DFAScopeRef scr1, DFAScopeRef scr2, bool copyAll)
    {
        ...
        while (!(lr1 = scr1.consumeNext(var)).isNull)
        {
            ...
            DFAScopeVar* scv3 = ret.assignLattice(var, meeted);
        }
        ...
        if (copyAll)
        {
            ...
            {
                lr2.check;
                DFAScopeVar* scv3 = ret.assignLattice(var, lr2);
            }
        }
    ...
    DFAScopeRef joinScope(DFAScopeRef scr1, DFAScopeRef scr2)
    {
        ...
        while (!(lr1 = scr1.consumeNext(var)).isNull)
        {
            ...
            DFAScopeVar* scv3 = ret.assignLattice(var, joined);
        }
            ...
            DFAVar* ignoreLHSCtx = null, bool ignoreWriteCount = false, bool unknownAware = false)
    {
        ...
        if (ignoreLHSCtx is null || lhsCtx.var !is ignoreLHSCtx)
        {
            foreach (c1; lhs)
            {
                if (c1.var is null)
        ...
        if (rhsCtx !is null && rhsCtx.var is null && lhsVar !is null)
        {
