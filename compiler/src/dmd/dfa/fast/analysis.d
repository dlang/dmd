...
struct DFAAnalyzer
{
    ...
    {
        DFAConsequence* assignToCctx = assignTo.getContext;
        DFAVar* assignToCtx = assignToCctx !is null ? assignToCctx.var : null;
        DFAConsequence* lrCctx = lr.getContext;
        DFAVar* lrCtx = lrCctx !is null ? lrCctx.var : null;
        ...
        {
            assignToCtx.visitDereferenceBases((contextVar) {
                if (!construct)
                {
                    if (noLR)
                ...
                assignTo2.setContext(contextVar);

                DFALatticeRef joined = join(assignTo2,
                    lr.copyWithoutInfiniteLifetime, 0, contextVar);
                DFAConsequence* c = joined.addConsequence(contextVar);
                joined.setContext(c);
                ...
                if (construct && isBlit)
                {
                    if (noLR)
                    {
                        ...
                        if (contextVar.isAPointer)
                            c.nullable = Nullable.Null;
                    }
                }
                else
                    seeWrite(contextVar, joined);
        ...
        if (assignToCtx !is null && assignToCtx.isStaticArray)
        {
        ...
        if (lrCtx !is null)
        {
            lrCtx.visitIfReferenceToAnotherVar((var) {
            ...
            DFAScopeRef scrFalse, DFALatticeRef lrFalse)
    {
...

        {
            ...
            asContext = dfaCommon.findVariablePair(trueVar, falseVar);
        }
