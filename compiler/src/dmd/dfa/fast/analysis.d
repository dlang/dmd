...
struct DFAAnalyzer
{
                    ...
                    if (DFAConsequence* outputCctx = outputParam.lr.getContext)
                    {
                        ...
                        if (!gotACell && outputCctx.var !is null)
                        {
                            outputCctx.var.visitIndirectSources((DFAVar* var,
                                    bool hadAnIndirection, bool hadAnOuterDeref,
    ...
    {
        DFAVar* contextVar = lr.getContextVar;

        // Does not call seeDereference, this is handled via onRead

        lr.setContext(dfaCommon.findDereferenceVar(contextVar));
        return lr;
    }
    ...
    void transferAddressOf(ref DFALatticeRef lr)
    {
        ...
        ctx.visitIndirectSources((DFAVar* var, bool hadAnIndirection, bool hadAnOuterDeref,
            ...
            bool isByRef = false, bool forMathOp = false)
    {
        ...
        contextVar.visitIndirectSources((DFAVar* root, bool hadAnIndirection, bool hadAnOuterDeref,
                bool takenAddressOf, bool hadFields, bool isOffsetOfStorage, ref bool unknown) {
            version (none)
            {
            ...
            if (root.isNullable && (hadAnIndirection || hadFields
                || isOffsetOfStorage || (hadAnOuterDeref && !takenAddressOf)))
