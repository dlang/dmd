...
struct DFACommon
{
    ...
    DFAVar* findDereferenceVar(DFAVar* childOf)
    {
        ...
        if (childOf.dereferenceVar is null)
        {
            ...
            childOf.dereferenceVar.base = childOf;
        }
...
struct DFAScopeRef
{
    ...
    DFAScopeRef copy()
    {
        ...
        {
            DFAScopeVar* scv2 = ret.sc.createAndInheritLattice(sc.dfaCommon, var, lr);
        }
            ...
            FuncDeclaration currentFunction = null, int depth = 0)
    {
        ...
        foreach (contextVar, l, scv; this)
        {
            ...
            if (contextVar !is null)
            {
                ...
                if (contextVar.base !is null)
                    printf(":%p", contextVar.base);
...
struct DFAConsequence
{
    ...
    // Ditto
    Nullable nullable;
    // Boolean logic can be inverted once, thanks to ||'s we won't be tracking it beyond that one invert.
    bool invertedOnce;
            ...
            FuncDeclaration currentFunction = null, int depth = 0, bool context = false)
    {
        ...
        printf("maybe=%p:%d, protectElseNegate=%d ", maybe, maybeTopSeen, protectElseNegate);

        printf("%p", this.var);
        if (this.var !is null)
        {
            printf("=%d:b%p", this.var.hasBeenAsserted, this.var.base);
