    ...
    void visitForeach(ForeachStatement fs)
    {
        ...
        case Tstruct:
                ...
                return retStmt(apply());
            {
                ...
                {
                    Expressions a;
                    if (auto f = resolveFuncCall(loc, sc, td, TemplateArguments(), tab, ArgumentList(&a), FuncResolveFlag.quiet))
                        tfront = f.type;
                }
...
private Statements* flatten(Statement statement, Scope* sc)
{
    ...
    switch(statement.stmt)
    {
        ...
        case STMT.Mixin:
            ...
            scope p = new Parser!ASTCodegen(cs.loc, sc._module, str, false, global.errorSink, &global.compileEnv, doUnittests);
