...
*/
private void lowerArrayAggregate(StaticForeach fs, Scope* sc)
{
    auto aggrfe = fs.aggrfe;
...
private extern(C++) void lowerNonArrayAggregate(StaticForeach fs, Scope* sc)
{
    auto aggrfe = fs.aggrfe;
    ...
    if (global.endGagging(olderrors))
        s2.push(fs.createForeach(aloc, pparams[1], null));
    else
    {
        s2.push(new ExpStatement(aloc, vard));
        auto catass = new CatAssignExp(aloc, new IdentifierExp(aloc, idres), res[1]);
        s2.push(fs.createForeach(aloc, pparams[1], new ExpStatement(aloc, catass)));
