    ...
    *pe0 = null;
    ...
    foreach (i, e; *ae.arguments)
    {
        ...
        if (auto ie = e.isIntervalExp())
        {
            ...
            sc = sc.push();
            FuncDeclaration fslice = resolveFuncCall(ae.loc, sc, slice, tiargs, ae.e1.type, fargs, null, FuncResolveFlag.quiet);
            sc = sc.pop();
...
private Expression resolvePropertiesX(Scope* sc, Expression e1, Expression e2 = null)
{
    ...
    Los:
        ...
        if (e2)
        {
            ...
            {
                if (FuncDeclaration f = resolveFuncCall(loc, sc, os.a[i], tiargs, tthis, &a, null, FuncResolveFlag.quiet))
                {
                    if (f.errors)
                        return ErrorExp.get();
                    fd = f;
        ...
        }
        {
            for (size_t i = 0; i < os.a.length; i++)
            {
                if (FuncDeclaration f = resolveFuncCall(loc, sc, os.a[i], tiargs, tthis, null, null, FuncResolveFlag.quiet))
                {
                    if (f.errors)
                        return ErrorExp.get();
                    fd = f;
    ...
    Lfd:
        ...
        if (e2)
        {
            ...
            FuncDeclaration fd = resolveFuncCall(loc, sc, s, tiargs, tthis, &a, null, FuncResolveFlag.quiet);
            if (fd && fd.type)
            {
                if (fd.errors)
                    return ErrorExp.get();
        ...
        {
            FuncDeclaration fd = resolveFuncCall(loc, sc, s, tiargs, tthis, null, null, FuncResolveFlag.quiet);
            if (fd && fd.type)
            {
                if (fd.errors)
                    return ErrorExp.get();
    ...
    Type* prettype, Expression* peprefix)
{
    ...
    *peprefix = null;
    ...
    if (names)
    {
        ...
        const(char)* msg = null;
        const result = tf.resolveNamedArgs(newArgs, names.peekSlice(), &msg);
        if (!result)
        {
...
private extern (C++) final class ExpressionSemanticVisitor : Visitor
{
    ...
    override void visit(NewExp exp)
    {
        ...
        if (auto tc = tb.isTypeClass())
        {
            ...
            if (cd.ctor)
            {
                FuncDeclaration f = resolveFuncCall(exp.loc, sc, cd.ctor, null, tb, exp.arguments, exp.names, FuncResolveFlag.standard);
                if (!f || f.errors)
                    return setError();
                ...
                if (!exp.arguments)
                    exp.arguments = new Expressions();
                if (functionParameters(exp.loc, sc, tf, null, exp.type, exp.arguments, exp.names, f, &exp.type, &exp.argprefix))
                    return setError();
        ...
        else if (auto ts = tb.isTypeStruct())
        {
            ...
            {
                FuncDeclaration f = resolveFuncCall(exp.loc, sc, sd.ctor, null, tb, exp.arguments, exp.names, FuncResolveFlag.standard);
                if (!f || f.errors)
                    return setError();
    ...
    Lagain:
        ...
        FuncDeclaration resolveOverloadSet(Loc loc, Scope* sc,
            OverloadSet os, Objects* tiargs, Type tthis, Expressions* arguments, Identifiers* names)
        {
            ...
            foreach (s; os.a)
            {
                if (tiargs && s.isFuncDeclaration())
                    continue;
                if (auto f2 = resolveFuncCall(loc, sc, s, tiargs, tthis, arguments, names, FuncResolveFlag.quiet))
                {
                    if (f2.errors)
                        return null;
                    if (f)
        ...
        if (exp.e1.op == EXP.dotVariable && t1.ty == Tfunction || exp.e1.op == EXP.dotTemplateDeclaration)
        {
            ...
            exp.f = resolveFuncCall(exp.loc, sc, s, tiargs, ue.e1.type, exp.arguments, exp.names, FuncResolveFlag.standard);
        ...
        else if (exp.e1.op == EXP.super_ || exp.e1.op == EXP.this_)
        {
            ...
            if (auto os = ctor.isOverloadSet())
                exp.f = resolveOverloadSet(exp.loc, sc, os, null, tthis, exp.arguments, exp.names);
            else
                exp.f = resolveFuncCall(exp.loc, sc, ctor, null, tthis, exp.arguments, exp.names, FuncResolveFlag.standard);
        ...
        {
            exp.f = resolveOverloadSet(exp.loc, sc, oe.vars, tiargs, tthis, exp.arguments, exp.names);
            if (!exp.f)
                return setError();
            if (ethis)
        ...
        else if (t1.ty != Tfunction)
        {
            ...
            else if (exp.e1.op == EXP.dotVariable && (cast(DotVarExp)exp.e1).var.isOverDeclaration())
            {
                ...
                exp.f = resolveFuncCall(exp.loc, sc, dve.var, tiargs, dve.e1.type, exp.arguments, exp.names, FuncResolveFlag.overloadOnly);
                if (!exp.f)
                    return setError();
                if (exp.f.needThis())
                {
            ...
            L2:
                exp.f = resolveFuncCall(exp.loc, sc, s, tiargs, null, exp.arguments, exp.names, FuncResolveFlag.standard);
                if (!exp.f || exp.f.errors)
                    return setError();
                if (exp.f.needThis())
                {
            ...
            if (!tf.callMatch(null, exp.arguments, exp.names, 0, &failMessage, sc))
            {
                OutBuffer buf;
                buf.writeByte('(');
        ...
        else if (VarExp ve = exp.e1.isVarExp())
        {
            ...
            if (exp.f.overnext)
                exp.f = resolveFuncCall(exp.loc, sc, exp.f, tiargs, null, exp.arguments, exp.names, FuncResolveFlag.overloadOnly);
            else
            {
            ...
            if (exp.f.needThis())
            {
                ...
                else if (isNeedThisScope(sc, exp.f))
                {
                    ...
                    if (exp.f.overnext)
                        exp.f = resolveFuncCall(exp.loc, sc, exp.f, tiargs, null, exp.arguments, exp.names, FuncResolveFlag.standard);
        ...
        if (!exp.arguments)
            exp.arguments = new Expressions();
        if (functionParameters(exp.loc, sc, cast(TypeFunction)t1, ethis, tthis, exp.arguments, exp.names, exp.f, &exp.type, &argprefix))
            return setError();
