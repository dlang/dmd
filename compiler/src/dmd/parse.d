...
class Parser(AST, Lexer = dmd.lexer.Lexer) : Lexer
{
  ...
  final:
    ...
    /********************************
     * Parse __sumtype(Type | Type | ...) as a type
     * Parser is sitting on the `__sumtype` token.
     */
    private AST.Type parseSumType()
    {
        nextToken();
        check(TOK.leftParenthesis, "`__sumtype`");
        ...
        if (token.value == TOK.rightParenthesis)
        {
            error("at least one type expected in `__sumtype(...)`");
            nextToken();
            return AST.Type.terror;
        }
        ...
        // Parse remaining types separated by `|`
        ...
        check(TOK.rightParenthesis, "`__sumtype`");
