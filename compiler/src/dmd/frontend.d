...
string determineDefaultCompiler()
{
    ...
    // Outer loop over compilers, inner over PATH entries (as before).
    ...
    // split PATH on the platform separator (no quote handling, as before)
    string[] dirs;
    size_t start = 0;
...
auto parseImportPathsFromConfig(const(char)[] iniFile, const(char)[] execDir)
{
    ...
    // search for all -I imports in this file (`-I[^ "]+`, possibly several
    // per line, exactly like the regex version)
    size_t i = 0;
    ...
    // remove duplicated import paths (sorted, as before)
    sortDedup(found);
...
string prettyPrint(Module m)
{
    ...
    // expand tabs (the buffer is uniquely owned, so a cast is safe)
...
/*
 * Private helpers for the compiler-discovery functions above.
 * They intentionally avoid phobos so that `dmd.frontend` stays usable
 * as a library without it; behavior matches the std.path/std.file based
...
// Mirrors std.path.dirName for well-formed paths.
private string dirNameOf(const(char)[] path)
{
    ...
    {
        // keep Windows drive roots like `C:\` and posix `/` intact
        if (end == 2 && path[1] == ':')
            break;
        if (end == 1)
            break;
    ...
    if (i == 0)
        return ".";
    if (i == 1)
        return path[0 .. 1].idup; // root "/" (or drive-relative edge, see below)
    size_t dirEnd = i - 1;
...
// Lexically normalize an import path (`.`/`..`/duplicate separators),
// mirroring std.path.buildNormalizedPath for well-formed inputs.
private string normalizeImportPath(const(char)[] path)
{
    if (!path.length)
        return "";
    // split into segments, tracking rootedness and drive prefix
    string prefix;
    size_t i = 0;
    version (Windows)
    {
        // drive letter prefix like `C:` (with or without following slash)
    ...
    while (i < path.length)
    {
        ...
        if (seg == "..")
        {
            if (parts.length && parts[$ - 1] != ".." && !(parts.length == 0 && rooted))
            {
                parts = parts[0 .. $ - 1];
                continue;
            }
...
// Expand `-I` argument: drop the prefix and substitute the common
// config variables with the compiler binary directory.
...
// Sort + dedup import paths in place (behavior matches `.array.sort.uniq`).
private void sortDedup(ref string[] paths)
{
    // insertion sort; import path lists are tiny
...
// Read a whole file; null on failure (no diagnostics reported).
