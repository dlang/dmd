...
public int runLINK()
{
    ...
    version (Windows)
    {
        ...
        if (target.mscoff)
        {
            ...
            foreach (const i; 0 .. global.params.objfiles.length)
            {
                cmdbuf.writeByte(' ');
            ...
            foreach (const i; 0 .. global.params.libfiles.length)
            {
                cmdbuf.writeByte(' ');
            ...
            foreach (const i; 0 .. global.params.linkswitches.length)
            {
                cmdbuf.writeByte(' ');
