...
void buildTransitiveImportClosure(ref Modules modules, ref bool[void*] closure)
{
    foreach (m; modules)
    {
        auto ptr = cast(void*)m;
        if (ptr !in closure)
        {
            closure[ptr] = true;
            buildTransitiveImportClosure(m.aimports, closure);
        }
    }
}
...
ubyte[20] generateCtfeCacheKey(FuncDeclaration fd, ref Expressions eargs)
{
    ...
    Modules roots;
    // Include all modules from type arguments
    ...
    roots.push(fd._scope._module);
    bool[void*] closure;
    buildTransitiveImportClosure(roots, closure);
    extern(C) static int moduleSignatureCmp(scope const void* a, scope const void* b)
    {
        auto ma = *cast(Module*)a;
        auto mb = *cast(Module*)b;
        foreach (i; 0..ma.signature.length)
        {
            int diff = cast(int)ma.signature[i] - mb.signature[i];
            if (diff != 0) return diff;
        }
        return 0;
    }
    // Set canonical order - for now by signature
    const(void)*[] mods = closure.keys();
    qsort(mods.ptr, mods.length, (void*).sizeof, &moduleSignatureCmp);
    debug(DCACHE) printf("DCache imports closure for %s:\n", signature);
    foreach (mod; mods)
    {
        auto m = cast(Module)mod;
        debug(DCACHE) auto str = toHexString(m.signature[]);
        debug(DCACHE) printf("\tmod %s signature %.*s\n", m.toChars(), str.length, str.ptr);
        hasher.put(m.signature[]);
    }
