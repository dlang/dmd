...
extern(C++) void genCppHdrFiles(ref Modules ms, const(char) *dir, const(char) *fname, bool only_c)
{
    initialize();
    ...
    scope OutBuffer hdr_guard;
    hdr_guard.writestring("DHDR_");
    if (only_c)
        hdr_guard.writestring("C_");
    else
        hdr_guard.writestring("CXX_");
    foreach (m; ms)
    {
        hdr_guard.writeByte('_');

        // write filename, all in uppercase, replacing non-alphabetics with underscores
        for (const(char) *x = m.srcfile.name; *x; x++)
        {
            if ('a' <= *x && *x <= 'z')
                hdr_guard.writeByte(*x + 'A' - 'a');
            else if ('A' <= *x && *x <= 'Z')
                hdr_guard.writeByte(*x);
            else
                hdr_guard.writeByte('_');
        }
    }
    ...
    buf.writestring(hdr_guard.peekChars);
    buf.writeByte('\n');
    ...
    OutBuffer done;
    OutBuffer decl;
    scope v = new ToCppBuffer!ASTCodegen(&check, &buf, &done, &decl, only_c);
    foreach (m; ms)
    {
...
`);
    ...
    else
    {
        ...
        const(char)[] name = FileName.combine(dir.toDString, fname.toDString);
...
public:
    ...
    this(OutBuffer* checkbuf, OutBuffer* fwdbuf, OutBuffer* donebuf, OutBuffer* buf, bool only_c)
    {
    ...
    override void visit(AST.FuncDeclaration fd)
    {
        ...
        if ((tf.linkage != LINK.c && tf.linkage != LINK.cpp) || (only_c && tf.linkage == LINK.cpp))
        {
    ...
    override void visit(AST.VarDeclaration vd)
    {
        ...
        {
            indent();
            if ((linkage != LINK.c && linkage != LINK.cpp) || (only_c && linkage == LINK.cpp))
            {
