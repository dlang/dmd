...
immutable string[] frontendSources = [
...
{
    return (cd.ident == Identifier.idPool("Identifier") &&
            cd.parent !is null &&
            cd.parent.ident == Identifier.idPool("identifier") &&
            cd.parent.parent && cd.parent.parent.ident == Identifier.idPool("dmd") &&
...
private bool isVisitorClass(ASTCodegen.ClassDeclaration cd)
{
    ...
    {
        if (cdb.ident == Identifier.idPool("Visitor") ||
            cdb.ident == Identifier.idPool("ParseTimeVisitor"))
           return true;
    }
    return false;
}
...
private bool isFrontendModule(ASTCodegen.Module m)
{
    ...
    // Ignore dmd.root
    if (m.parent.ident == Identifier.idPool("root") &&
        m.parent.parent && m.parent.parent.ident == Identifier.idPool("dmd") &&
    ...
    if ((m.ident == Identifier.idPool("visitor") ||
         m.ident == Identifier.idPool("parsetimevisitor") ||
         m.ident == Identifier.idPool("permissivevisitor") ||
         m.ident == Identifier.idPool("strictvisitor") ||
         m.ident == Identifier.idPool("transitivevisitor")) &&
        m.parent && m.parent.ident == Identifier.idPool("dmd") &&
        !m.parent.parent)
    {
        return false;
    }

    return ((m.parent.ident == Identifier.idPool("dmd") && !m.parent.parent) ||
            (m.parent.parent.ident == Identifier.idPool("dmd") && !m.parent.parent.parent));
}
...
void genCppFiles(OutBuffer* buf, Modules *ms)
{
    ...
    public:
        ...
        private void enumToBuffer(AST.EnumDeclaration ed)
        {
            if (ed.isSpecial())
            {
                if (ed.ident == Identifier.idPool("__c_long"))
                    buf.writestring("long");
                else if (ed.ident == Identifier.idPool("__c_ulong"))
                    buf.writestring("unsigned long");
                else if (ed.ident == Identifier.idPool("__c_longlong"))
                    buf.writestring("long long");
                else if (ed.ident == Identifier.idPool("__c_ulonglong"))
                    buf.writestring("unsigned long long");
                else if (ed.ident == Identifier.idPool("__c_long_double"))
        ...
        {
            if (ti.tempdecl.ident == Identifier.idPool("AssocArray"))
            {
                buf.writestring("AA*");
                return;
            }
            if (ti.tempdecl.ident == Identifier.idPool("Array"))
...
void main()
{
    ...
    global.params.fileImppath = global.filePath;
