...
extern (C++) bool checkAccess(AggregateDeclaration ad, Loc loc, Scope* sc, Dsymbol smember)
{
    ...
    if (!result && (!(sc.flags & SCOPE.onlyunsafeaccess) || sc.func.setUnsafe()))
    {
        ad.error(loc, "member `%s` is not accessible%s", smember.toChars(), (sc.flags & SCOPE.onlyunsafeaccess) ? " from `@safe` code".ptr : "".ptr);
