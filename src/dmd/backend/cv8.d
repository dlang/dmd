...
extern __gshared int BPoff;               // offset from BP
extern __gshared LocalSection Auto;       // section of automatics and registers
...
void cv8_writename(Outbuffer *buf, const char* name, size_t len)
{
    ...
    {
        char* start = cast(char*) name; // TODO
        char* cur = strchr(start, '.');
...
void cv8_termfile(const char *objfilename)
{
    ...
    buf.writeByte(VERSION.sizeof);
    buf.writeByte('Z');
    buf.write(VERSION.ptr, cast(uint) VERSION.sizeof - 1);
...
void cv8_func_term(Symbol *sfunc)
{
    ...
    buf.reserve(2 + 2 + 4 * 7 + 6 + 1 + cast(uint) len + 1);
    buf.writeWordn( 2 + 4 * 7 + 6 + 1 + cast(uint) len + 1);
    ...
    struct cv8
    {
        ...
        private static void endArgs()
        {
            Outbuffer *buf = currentfuncdata.f1buf;
            buf.writeWord(2);
            buf.writeWord(S_ENDARG);
        }
        private static void beginBlock(int offset, int length)
        {
            Outbuffer *buf = currentfuncdata.f1buf;
            uint soffset = cast(uint) buf.size();
            // parent and end to be filled by linker
            block_v3_data block32 = { len:block_v3_data.sizeof - 2, S_BLOCK_V3, 0, 0, length, offset, 0, [0] };
