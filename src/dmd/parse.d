...
final class Parser(AST)
{
    ...
    {
        lexer = new Lexer(_module ? _module.srcfile.toChars() : null, input.ptr, 0, input.length, doDocComment, false, diagnosticReporter);
    ...
    AST.Dsymbols* parseModule()
    {
        ...
        Token* tk;
        if (skipAttributes(&lexer.token, &tk) && tk.value == TOK.module_)
        {
    ...
    {
        if (peek(&lexer.token).value != TOK.leftParentheses)
    ...
    AST.Dsymbols* parseDeclDefs(int once, AST.Dsymbol* pLastDecl = null, PrefixAttributes!AST* pAttrs = null)
    {
        ...
        do
        {
            ...
            Lautodecl:
                 ...
                 *      storage_class identifier(...) = initializer;
                 */
                if (token.value == TOK.identifier && skipParensIf(peek(&lexer.token), &tk) && tk.value == TOK.assign)
                {
                 ...
                 */
                if (token.value == TOK.identifier && skipParens(peek(&lexer.token), &tk) && skipAttributes(tk, &tk) &&
        ...
        if (flag || token.value != TOK.rightParentheses)
        {
            ...
            while (token.value != TOK.rightParentheses)
            {
                ...
                if (token.value == TOK.alias_)
                {
                    ...
                    if (isDeclaration(&lexer.token, NeedDeclaratorId.must, TOK.reserved, null))
                    {
                    ...
                    {
                        nextToken();
                        if (isDeclaration(&lexer.token, NeedDeclaratorId.no, TOK.reserved, null))
                            spec = parseType();
                        else
                            spec = parseCondExp();
                    }
    ...
    private AST.Dsymbol parseCtor(PrefixAttributes!AST* pAttrs)
    {
         ...
         */
        AST.TemplateParameters* tpl = null;
        if (token.value == TOK.leftParentheses && peekPastParen(&lexer.token).value == TOK.leftParentheses)
        {
            tpl = parseTemplateParameterList();
        }
            ...
            case TOK.leftBracket:
                {
                    ...
                    else if (isDeclaration(&lexer.token, NeedDeclaratorId.no, TOK.rightBracket, null))
                    {
            ...
            case TOK.leftParentheses:
                {
                    if (tpl)
                    {
                        Token* tk = peekPastParen(&lexer.token);
    ...
    private AST.Dsymbols* parseDeclarations(bool autodecl, PrefixAttributes!AST* pAttrs, const(char)* comment)
    {
                ...
                while (1)
                {
                    ...
                    if (token.value == TOK.function_ ||
                        ...
                        token.value == TOK.leftParentheses &&
                            skipAttributes(peekPastParen(&lexer.token), &tk) &&
         ...
         *  storage_class identifier(...) = initializer;
         */
        if ((storage_class || udas) && token.value == TOK.identifier && skipParensIf(peek(&lexer.token), &tk) && tk.value == TOK.assign)
        {
        ...
        /* Look for return type inference for template functions.
         */
        if ((storage_class || udas) && token.value == TOK.identifier && skipParens(peek(&lexer.token), &tk) &&
    ...
    AST.Statement parseStatement(int flags, const(char)** endPtr = null, Loc* pEndloc = null)
    {
        ...
        case TOK.traits:
             ...
             */
            if (isDeclaration(&lexer.token, NeedDeclaratorId.mustIfDstyle, TOK.reserved, null))
                goto Ldeclaration;
            else
                goto Lexp;
        ...
        case TOK.if_:
            ...
            LagainStc:
                ...
                if (storageClass != 0 && token.value == TOK.identifier && peek(&lexer.token).value == TOK.assign)
                {
                ...
                else if (isDeclaration(&lexer.token, NeedDeclaratorId.must, TOK.assign, null))
                {
                    Identifier ai;
        ...
        case TOK.asm_:
            {
                ...
                while (1)
                {
                    ...
                    default:
                        *ptoklist = lexer.allocateToken();
                        memcpy(*ptoklist, &lexer.token, Token.sizeof);
    ...
    private AST.Initializer parseInitializer()
    {
        ...
        case TOK.leftCurly:
             ...
             */
            braces = 1;
            for (t = peek(&lexer.token); 1; t = peek(t))
            {
                switch (t.value)
                {
                ...
                if (token.value == TOK.leftParentheses)
                {
                    ...
                    if (token.value == TOK.colon || token.value == TOK.equal)
                    {
                        ...
                        if (tok == TOK.equal && (token.value == TOK.struct_ || token.value == TOK.union_
                            ...
                            || token.value == TOK.parameters || token.value == TOK.const_ && peek(&lexer.token).value == TOK.rightParentheses
                            || token.value == TOK.immutable_ && peek(&lexer.token).value == TOK.rightParentheses
                            || token.value == TOK.shared_ && peek(&lexer.token).value == TOK.rightParentheses
                            || token.value == TOK.inout_ && peek(&lexer.token).value == TOK.rightParentheses || token.value == TOK.function_
                            || token.value == TOK.delegate_ || token.value == TOK.return_
                            || (token.value == TOK.vector && peek(&lexer.token).value == TOK.rightParentheses)))
                        {
                            tok2 = token.value;
                            nextToken();
                        }
    ...
    private void addComment(AST.Dsymbol s, const(char)* blockComment)
    {
        if (s !is null)
        {
            s.addComment(lexer.combineComments(blockComment, token.lineComment, true));
    ...
    package ref Loc scanloc() pure nothrow @nogc @safe
    {
        return lexer.scanloc;
    }

    package void scanloc(const ref Loc loc) pure nothrow @nogc @safe
    {
        lexer.scanloc = loc;
    }

    private Token* peek(Token* currentToken)
    {
        return lexer.peek(currentToken);
    }

    package ref Token token() pure nothrow @nogc @safe
    {
        return lexer.token;
    }
    ...
    package TOK peekNext()
    {
        return lexer.peekNext();
    }

    private TOK peekNext2()
    {
        return lexer.peekNext2();
    }

    private Token* peekPastParen(Token* token)
    {
        return lexer.peekPastParen(token);
    }
    ...
    package Token* allocateToken() pure nothrow @safe
    {
        return lexer.allocateToken();
    }

    package void error(const(char)* format, ...)
    {
        va_list args;
        va_start(args, format);
        lexer.diagnosticReporter.error(lexer.token.loc, format, args);
        va_end(args);
    }

    package void error(const ref Loc loc, const(char)* format, ...)
    {
        va_list args;
        va_start(args, format);
        lexer.diagnosticReporter.error(loc, format, args);
        va_end(args);
    }

    private void errorSupplemental(const ref Loc loc, const(char)* format, ...)
    {
        va_list args;
        va_start(args, format);
        lexer.diagnosticReporter.errorSupplemental(loc, format, args);
        va_end(args);
    }

    private void warning(const ref Loc loc, const(char)* format, ...)
    {
        va_list args;
        va_start(args, format);
        lexer.diagnosticReporter.warning(loc, format, args);
        va_end(args);
    }

    private void warningSupplemental(const ref Loc loc, const(char)* format, ...)
    {
        va_list args;
        va_start(args, format);
        lexer.diagnosticReporter.warningSupplemental(loc, format, args);
        va_end(args);
    }

    private void deprecation(const(char)* format, ...)
    {
        va_list args;
        va_start(args, format);
    ...
    private void deprecation(const ref Loc loc, const(char)* format, ...)
    {
        va_list args;
        va_start(args, format);
        lexer.diagnosticReporter.deprecation(loc, format, args);
        va_end(args);
    }

    private void deprecationSupplemental(const ref Loc loc, const(char)* format, ...)
    {
        va_list args;
        va_start(args, format);
        lexer.diagnosticReporter.deprecationSupplemental(loc, format, args);
        va_end(args);
    }
}
