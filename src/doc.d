    ...
    void write(Loc loc, DocComment* dc, Scope* sc, Dsymbols* a, OutBuffer* buf)
    {
        ...
        if (namelen)
        {
            static __gshared const(char)** table = [
                "AUTHORS", "BUGS", "COPYRIGHT", "DATE", "DEPRECATED",
                "EXAMPLES", "HISTORY", "LICENSE", "RETURNS", "SEE_ALSO",
                "STANDARDS", "THROWS", "VERSION", null
            ];
                ...
                buf.writestring("$(DDOC_PARAM_ROW ");
                {
                    buf.writestring("$(DDOC_PARAM_ID ");
                    {
                        ...
                        else
                        {
                            ...
                            {
                                warning(s.loc,
                                    "Ddoc: function declaration has no parameter '%.*s'",
                                    namelen, namestart);
                            }
...
extern (C++) void gendocfile(Module m)
{
    ...
    Macro.define(&m.macrotable, cast(char*)"SRCFILENAME", 11,
        cast(char*)srcfilename, strlen(srcfilename));
    char* docfilename = m.docfile.toChars();
    Macro.define(&m.macrotable, cast(char*)"DOCFILENAME", 11,
        cast(char*)docfilename, strlen(docfilename));
    if (dc.copyright)
    {
        dc.copyright.nooutput = 1;
        Macro.define(&m.macrotable, cast(char*)"COPYRIGHT", 9,
            dc.copyright._body, dc.copyright.bodylen);
    }
    ...
    for (size_t u = start; u < buf.offset; u++)
    {
        ...
        case ')':
            if (par_open == 0)
            {
                //stray ')'
                warning(
                    loc, "Ddoc: Stray ')'. This may cause incorrect Ddoc output. Use $(RPAREN) instead for unpaired right parentheses.");
                buf.remove(u, 1); //remove the )
                buf.insert(u, cast(const(char)*)"$(RPAREN)", 9); //insert this instead
                u += 8; //skip over newly inserted macro
            }
        ...
        for (size_t u = buf.offset; u > start;)
        {
            ...
            case '(':
                if (par_open == 0)
                {
                    //stray '('
                    warning(
                        loc, "Ddoc: Stray '('. This may cause incorrect Ddoc output. Use $(LPAREN) instead for unpaired left parentheses.");
                    buf.remove(u, 1); //remove the (
                    buf.insert(u, cast(const(char)*)"$(LPAREN)", 9); //insert this instead
                }
                else
...
extern (C++) static void emitAnchor(OutBuffer* buf, Dsymbol s, Scope* sc)
{
    ...
    if (*count > 0 && sc.prevAnchor == ident && sc.lastdc && (isDitto(s.comment) ||
            (td && isDitto(td.comment))))
        return;
    (*count)++;
    // cache anchor name
    sc.prevAnchor = ident;
...
extern (C++) static void expandTemplateMixinComments(TemplateMixin tm, OutBuffer* buf,
    Scope* sc)
{
    if (!tm.semanticRun)
        tm.semantic(sc);
    ...
    public:
        ...
        override void visit(Declaration d)
        {
            ...
            else
            {
                ...
                if (!d.type)
                {
                    if (!d.isCtorDeclaration() && !d.isAliasDeclaration() && !d.isVarDeclaration())
                    {
                        return;
                    }
                }
    ...
    public:
        ...
        void prefix(Dsymbol s)
        {
            ...
            if (Declaration d = s.isDeclaration())
            {
                ...
                if (!d.type && d.isVarDeclaration() && !d.isImmutable() &&
                        !(d.storage_class & STCshared) && !d.isWild() &&
                        !d.isConst() && !d.isSynchronized())
                {
        ...
        override void visit(Declaration d)
        {
            ...
            if (d.type)
            {
                ...
                {
                    functionToBufferFull(cast(TypeFunction)origType, buf, d.ident,
                        &hgs, td);
                }
                else
...
struct DocComment
{
     ...
     *      name2 = value2
     */
    extern (C++) static void parseMacros(Escape** pescapetable,
        Macro** pmacrotable, const(char)* m, size_t mlen)
    {
        const(char)* p = m;
        size_t len = mlen;
        const(char)* pend = p + len;
     ...
     */
    extern (C++) static void parseEscapes(Escape** pescapetable,
        const(char)* textstart, size_t textlen)
    {
...
extern (C++) bool isReservedName(char* str, size_t len)
{
    static __gshared const(char)** table = [
        ...
        "__returnLabel", "__vptr", "__monitor", "__gate", "__xopEquals",
        "__xopCmp", "__LINE__", "__FILE__", "__MODULE__", "__FUNCTION__",
        "__PRETTY_FUNCTION__", "__DATE__", "__TIME__", "__TIMESTAMP__",
        "__VENDOR__", "__VERSION__", "__EOF__", "__LOCAL_SIZE",
        "___tls_get_addr", "__entrypoint", "__va_argsave_t", "__va_argsave", null
    ];
    for (int i = 0; table[i]; i++)
    {
