...
{   Expression *e;
    ...
    if (e2->type->isintegral())
    {
        ...
        if (e1->type->isfloating())
        {
            ...
            v = new RealExp(loc, to_real(1.0), e1->type);
        }
        else
        {
            r = new RealExp(loc, e1->toReal(), Type::tfloat64);
            v = new RealExp(loc, to_real(1.0), Type::tfloat64);
        }
        ...
        if (neg)
            v = Div(v->type, new RealExp(loc, to_real(1.0), v->type), v);
    ...
    else if (e2->type->isfloating())
    {
        ...
        if (e1->toReal() < 0.0)
        {
            e = new RealExp(loc, to_real(Port::nan), type);
        }
