...
/**
 * Should be called by iterateAliasThis inside resloveAliasThisForBinExp.
 ...
 */
static bool atSubstBinLhs(Scope *sc, Expression *e, void *ctx, Expression **outexpr)
{
    BinExp *be = (BinExp *)((BinExp*)ctx)->copy();
    be->e1 = e;
    unsigned oldatlock1 = be->e1->type->aliasthislock;
    unsigned oldatlock2 = be->e2->type->aliasthislock;
    be->e1->type->aliasthislock |= RECtracing;
    be->e2->type->aliasthislock |= RECtracing;
 ...
 */
static bool atSubstBinRhs(Scope *sc, Expression *e, void *ctx, Expression **outexpr)
{
    BinExp *be = (BinExp *)((BinExp*)ctx)->copy();
    be->e2 = e;
    unsigned oldatlock1 = be->e1->type->aliasthislock;
 ...
 * bin(x, e) -> bin(x, e.aliasthis)
 */
static bool atSubstBinBoth(Scope *sc, Expression *e, void *ctx_, Expression **outexpr)
{
    ...
    int ret = iterateAliasThis(sc, be->e2, &atSubstBinRhs, (void*)be, ctx->results);
...
void resloveAliasThisForBinExp(Scope *sc, BinExp *be, bool check_lvl, bool check_rvl, Expressions* ret)
{
    if (check_lvl)
    {
        ...
        iterateAliasThis(sc, be->e1, &atSubstBinLhs, (void*)be, ret);
    }

    if (check_rvl)
    {
        // e1 op e2 => e1 op e2.aliasthis
        iterateAliasThis(sc, be->e2, &atSubstBinRhs, (void*)be, ret);
    }
...
Expression *op_overload(Expression *e, Scope *sc)
{
    ...
    public:
        ...
        void visit(UnaExp *e)
        {
            ...
            if (e->e1->op == TOKarray)
            {
                ...
                if (ad)
                {
                    ...
                    if (!e->aliasthislock)
                    {
                        ...
                        Expressions results;
                        iterateAliasThis(sc, ae->e1, &atSubstUnaUna, (void*)e, &results);
                        if (results.dim == 1)
                        {
                            result = results[0];
                            return;
            ...
            else if (e->e1->op == TOKslice)
            {
                ...
                if (ad)
                {
                    ...
                    if (!e->aliasthislock)
                    {
                        ...
                        Expressions results;
                        iterateAliasThis(sc, se->e1, &atSubstUnaUna, (void*)ue, &results);
                        if (results.dim == 1)
                        {
                            result = results[0];
                            return;
            ...
            if (ad)
            {
                ...
                if (!e->aliasthislock)
                {
                    ...
                    Expressions results;
                    iterateAliasThis(sc, e->e1, &atSubstUna, (void*)e, &results);
                    if (results.dim == 1)
                    {
                        result = results[0];
                        return;
        ...
        void visit(ArrayExp *ae)
        {
            ...
            Lslice:
                ...
                if (!ae->aliasthislock)
                {
                    ...
                    Expressions results;
                    iterateAliasThis(sc, ae->e1, &atSubstUna, (void*)ae, &results);
                    if (results.dim == 1)
                    {
                        result = results[0];
                        return;
        ...
        L1:
            ...
            if (!(e->op == TOKassign && ad2 && ad1 == ad2) && !e->aliasthislock)   // See Bugzilla 2943
            {
                check_lvl = true;
            }
        ...
        void visit(BinAssignExp *e)
        {
            ...
            if (e->e1->op == TOKarray)
            {
                ...
                if (ad)
                {
                    ...
                    {
                        Expressions results;
                        iterateAliasThis(sc, ae->e1, &atSubstBinUna, (void*)e, &results);
            ...
            else if (e->e1->op == TOKslice)
            {
                ...
                if (ad)
                {
                    ...
                    {
                        Expressions results;
                        iterateAliasThis(sc, se->e1, &atSubstBinUna, (void*)e, &results);
 ...
 */
static bool atSubstForeach(Scope *sc, Expression *e, void *ctx, Expression **outexpr)
{
    ...
    while (1)
    {
        ...
        switch (tab->ty)
        {
            ...
            Laggr:
                ...
                {
                    Expressions results;
                    iterateAliasThis(sc, aggr, &atSubstForeach, (void*)fes, &results);
                    ...
                    {
                        aggr->error("Unable to unambiguously resolve %s Candidates:", aggr->toChars());
                        for (size_t j = 0; j < results.dim; ++j)
                        {
                            aggr->error("%s", results[j]->toChars());
                        }
                        aggr = new ErrorExp();
                        continue;
                    }
