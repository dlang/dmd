...
public:
    ...
    override final void semantic3(Scope* sc)
    {
        ...
        // or error types
        if (!getRTInfo && Type.rtinfo && (!isDeprecated() ||
                global.params.useDeprecated) && (type && type.ty != Terror))
        {
            ...
            struct SV
            {
                ...
                extern (C++) static int func(Dsymbol s, void* param)
                {
                    ...
                    if (v)
                    {
                        ...
                        if (
                                v.storage_class & (
                                STCstatic | STCextern | STCtls | STCgshared | STCmanifest | STCctfe | STCtemplateparameter))
                            return 0;
     ...
     */
    final static uint placeField(uint* nextoffset, uint memsize,
        uint memalignsize, structalign_t alignment, uint* paggsize,
        uint* paggalignsize, bool isunion)
    {
        uint ofs = *nextoffset;
    ...
    final Dsymbol searchCtor()
    {
        ...
        if (s)
        {
            ...
            {
                s.error(
                    "is not a constructor; identifiers starting with __ are reserved for the implementation");
                errors = true;
                s = null;
            }
        }
